#!/usr/bin/env python3
"""
HPC Maturity Assessment Agent - GUI (POTPUNO POPRAVLJENA VERZIJA)
====================================
Grafički korisnički interfejs za procjenu HPC zrelosti.

ISPRAVKE:
- Dodat error handling u _save_report i _export_results metodi
- Promijenjeno 'initialfilename' u 'initialfile' za Python 3.13 kompatibilnost
- Dodati detaljni error messages
- Fiksiran problem sa čuvanjem izvještaja
"""
...
Ukupni kapacitet:
   ...
   - Procijenjeni TFLOPS: ~{total_cores * 0.05:.1f} (CPU) + ~{total_gpus * 10:.1f} (GPU)
    ...
    def _run_assessment(self):
        ...
        def run():
            try:
                ...
                self.root.after(0, lambda: messagebox.showerror("Greška", error_msg))
            finally:
    ...
    def _save_report(self):
        """Save report to file - FIXED VERSION with proper error handling."""
        # Provjera da li je procjena izvrsena
        if not self.agent.assessment_complete:
            messagebox.showinfo("Info", "Prvo pokrenite procjenu.")
            return

        # Dodatna provjera da li postoji assessment_output
        if not self.assessment_output:
            messagebox.showerror("Greška", "Nema dostupnih rezultata za čuvanje. Molimo pokrenite procjenu ponovo.")
            return

        # Otvori save dialog - FIXED: initialfile umjesto initialfilename
        filepath = filedialog.asksaveasfilename(
            defaultextension=".txt",
            filetypes=[("Text files", "*.txt"), ("All files", "*.*")],
            initialfile="hpc_assessment_report.txt"  # FIXED: bilo initialfilename
        )

        # Provjeravamo da li je korisnik izabrao fajl
        if not filepath:
            return  # Korisnik je kliknuo Cancel

        # Pokusavamo da sacuvamo fajl sa error handling-om
        try:
            with open(filepath, 'w', encoding='utf-8') as f:
                # Pišemo maturity map
                try:
                    maturity_map = self.agent.get_maturity_map()
                    f.write(maturity_map)
                    f.write("\n\n")
                except Exception as e:
                    f.write(f"[Greška pri generisanju maturity map: {str(e)}]\n\n")

                # Pišemo summary
                try:
                    if hasattr(self.assessment_output, 'summary') and self.assessment_output.summary:
                        f.write(self.assessment_output.summary)
                        f.write("\n\n")
                    else:
                        f.write("[Summary nije dostupan]\n\n")
                except Exception as e:
                    f.write(f"[Greška pri pisanju summary-ja: {str(e)}]\n\n")

                # Pišemo detailed report
                try:
                    detailed_report = self.agent.get_detailed_report()
                    f.write(detailed_report)
                except Exception as e:
                    f.write(f"[Greška pri generisanju detaljnog izvještaja: {str(e)}]\n")

            # Uspješno sačuvan fajl
            messagebox.showinfo("Uspjeh", f"Izvještaj sačuvan:\n{filepath}")

        except PermissionError:
            messagebox.showerror(
                "Greška pri čuvanju",
                f"Nemate dozvolu za pisanje u:\n{filepath}\n\nMolimo izaberite drugu lokaciju."
            )
        except IOError as e:
            messagebox.showerror(
                "Greška pri čuvanju",
                f"Greška pri pisanju fajla:\n{str(e)}"
            )
        except Exception as e:
            messagebox.showerror(
                "Neočekivana greška",
                f"Došlo je do neočekivane greške:\n{str(e)}\n\nTraceback:\n{traceback.format_exc()}"
            )

    def _export_results(self):
        """Export results to JSON - FIXED VERSION."""
        if not self.agent.assessment_complete:
            messagebox.showinfo("Info", "Prvo pokrenite procjenu.")
            return

        # FIXED: initialfile umjesto initialfilename
        filepath = filedialog.asksaveasfilename(
            defaultextension=".json",
            filetypes=[("JSON files", "*.json"), ("All files", "*.*")],
            initialfile="hpc_assessment_results.json"  # FIXED: bilo initialfilename
        )
        ...
        if filepath:
            try:
                ...
                messagebox.showinfo("Uspjeh", f"Rezultati eksportovani:\n{filepath}")
            except PermissionError:
                messagebox.showerror(
                    "Greška pri eksportovanju",
                    f"Nemate dozvolu za pisanje u:\n{filepath}"
                )
            except Exception as e:
                messagebox.showerror(
